...
# считаем более правильно произведение: gradPerm * gradP = dk/dx*dp/dx + dk/dy*dp/dy
...
#perm_matrix = np.random.randint(low=1,high=100,size=(N_r_full,M_fi_full)) * 10**(-15)
perm_matrix = np.ones((N_r_full, M_fi_full)) * perm
for m in range(0, M_fi_full):
    for n in range(0, N_r_full):
        if m == 10 or m == 20:
            perm_matrix[n][m] = perm_matrix[n][m]*100
            perm_matrix[n][m+1] = perm_matrix[n][m+1] * 100
            perm_matrix[n][m+2] = perm_matrix[n][m+2] * 100
perm_matrix[20:50, 100:150] = perm*100
courant_number_min = 2*delta_t*np.min(perm_matrix)/fi/cf/mu/delta_x**2
courant_number_max = 2*delta_t*np.max(perm_matrix)/fi/cf/mu/delta_x**2
print(courant_number_min, courant_number_max)
#div_perm_matrix = np.gradient(perm_matrix)
div_perm_matrix = [np.zeros((N_r_full, M_fi_full)), np.zeros((N_r_full, M_fi_full))]
# div_perm_matrix = np.zeros((N_r_full, M_fi_full))
for m in range(0, M_fi_full):
    for n in range(0, N_r_full):
        if m == 0 and n == 0:
            div_perm_matrix[0][n][m] = (perm_matrix[n+1][m]-perm_matrix[n][m])/delta_x
            div_perm_matrix[1][n][m] = + (perm_matrix[n][m + 1] - perm_matrix[n][m]) / delta_y
        elif m == 0 and n == N_r_full-1:
            div_perm_matrix[0][n][m] = (perm_matrix[n][m]-perm_matrix[n-1][m])/delta_x
            div_perm_matrix[1][n][m] = + (perm_matrix[n][m + 1] - perm_matrix[n][m]) / delta_y
        elif m == M_fi_full-1 and n == 0:
            div_perm_matrix[0][n][m] = (perm_matrix[n+1][m]-perm_matrix[n][m])/delta_x
            div_perm_matrix[1][n][m] = + (perm_matrix[n][m] - perm_matrix[n][m - 1]) / delta_y
        elif m == M_fi_full-1 and n == N_r_full-1:
            div_perm_matrix[0][n][m] = (perm_matrix[n][m]-perm_matrix[n-1][m])/delta_x
            div_perm_matrix[1][n][m] = + (perm_matrix[n][m] - perm_matrix[n][m - 1]) / delta_y
        elif m == 0:
            div_perm_matrix[0][n][m] = (perm_matrix[n+1][m]-perm_matrix[n-1][m])/2/delta_x
            div_perm_matrix[1][n][m] = + (perm_matrix[n][m + 1] - perm_matrix[n][m]) / delta_y
        elif m == M_fi_full-1:
            div_perm_matrix[0][n][m] = (perm_matrix[n+1][m]-perm_matrix[n-1][m])/2/delta_x
            div_perm_matrix[1][n][m] = + (perm_matrix[n][m] - perm_matrix[n][m - 1]) / delta_y
        elif n == 0:
            div_perm_matrix[0][n][m] = (perm_matrix[n+1][m]-perm_matrix[n][m])/2/delta_x
            div_perm_matrix[1][n][m] = + (perm_matrix[n][m + 1] - perm_matrix[n][m - 1]) / delta_y
        elif n == N_r_full-1:
            div_perm_matrix[0][n][m] = (perm_matrix[n][m]-perm_matrix[n-1][m])/2/delta_x
            div_perm_matrix[1][n][m] = + (perm_matrix[n][m + 1] - perm_matrix[n][m - 1]) / delta_y
        else:
            div_perm_matrix[0][n][m] = (perm_matrix[n+1][m]-perm_matrix[n-1][m])/2/delta_x
            div_perm_matrix[1][n][m] = + (perm_matrix[n][m + 1] - perm_matrix[n][m - 1]) / 2 / delta_y
div_perm_matrix = np.gradient(perm_matrix)
...
def PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, Pres_left, Pres_right, Pres_top, Pres_bottom, CP_dict,
    ...
    for m in range(M_fi_full):
        ...
        for n in range(1, N_r_full - 1):
            A[n][n - 1] = -div_perm_matrix[0][n][m]/2/delta_x + perm_matrix[n][m]/delta_x**2
        ...
        A[0][1] = div_perm_matrix[0][0][m]/2/delta_x + perm_matrix[0][m]/delta_x**2

        A[N_r_full - 1][N_r_full - 1] = -2*perm_matrix[N_r_full-1][m]/delta_x**2 - 2*perm_matrix[N_r_full-1][m]/delta_y**2 - fi*mu*cf/delta_t
        A[N_r_full - 1][N_r_full - 2] = -div_perm_matrix[0][N_r_full-1][m]/2/delta_x + perm_matrix[N_r_full-1][m]/delta_x**2

        A_sym_rigtht = np.zeros((N_r_full, N_r_full))
        A_sym_left = np.zeros((N_r_full, N_r_full))
        for n in range(0, N_r_full):
            A_sym_rigtht[n][n] = div_perm_matrix[1][n][m]/2/delta_y + perm_matrix[n][m]/delta_y**2
            A_sym_left[n][n] = -div_perm_matrix[1][n][m]/2/delta_y + perm_matrix[n][m]/delta_y**2
        ...
        for n in range(N_r_full):
            if m == 0 and n == 0:
                B[j][0] = -fi*mu*cf/delta_t * Pres_distrib[n][m] - \
                          (-div_perm_matrix[0][n][m]/2/delta_x + perm_matrix[n][m]/delta_x**2)*Pres_left + \
                          (div_perm_matrix[1][n][m]/2/delta_y - perm_matrix[n][m]/delta_y**2)*Pres_bottom
            elif m == 0 and n == N_r_full-1:
                B[j][0] = -fi*mu*cf/delta_t * Pres_distrib[n][m] - \
                          (div_perm_matrix[0][n][m]/2/delta_x + perm_matrix[n][m]/delta_x**2)*Pres_right + \
                          (div_perm_matrix[1][n][m]/2/delta_y - perm_matrix[n][m]/delta_y**2)*Pres_bottom
            elif m == M_fi_full-1 and n == 0:
                B[j][0] = -fi*mu*cf/delta_t * Pres_distrib[n][m] - \
                          (-div_perm_matrix[0][n][m]/2/delta_x + perm_matrix[n][m]/delta_x**2)*Pres_left - \
                          (div_perm_matrix[1][n][m]/2/delta_y + perm_matrix[n][m]/delta_y**2)*Pres_top
