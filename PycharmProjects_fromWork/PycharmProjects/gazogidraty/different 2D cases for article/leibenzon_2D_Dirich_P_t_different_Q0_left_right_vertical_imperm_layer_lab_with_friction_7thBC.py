...
P_bottom_x_t = []
#t_list = [11.7, 11.8, 11.9, 12, 12.1, 12.2, 12.3, 12.4, 12.5, 12.6, 12.65]
t_list_1 = [11.7, 11.8, 11.9, 12, 12.1, 12.2]
P_bound_list_1 = [(0.0409*elem + 0.5472)*1.01325*10**5 for elem in t_list_1]
#P_bound_list_1 = [4354.2*elem + 55526 for elem in t_list_1]
t_list_2 = [12.9, 12.9, 12.9, 12.9, 12.9, 12.9]
P_bound_list_2 = [(0.0409*elem + 0.5472)*1.01325*10**5 for elem in t_list_2]
...
for t in range(T):
    ...
    for n in range(N_r_full):
        if n == round(N_r_full/2) or n == round(N_r_full/2)+1 or n == round(N_r_full/2)+2:
            P_bottom_x.append(P_bound_list_1[t]) # если поставить 1.11 то будет разрыв в одной точке соседней с полоской
        else:
            P_bottom_x.append(P_bound_list_1[t])
    P_bottom_x_t.append(P_bottom_x)

#P_bottom_t = [1.066*10**5 for i in range(T)]
viscosity_matrix = np.ones((N_r_full, M_fi_full))*mu_water

imperm_layer_X_position = int(N_r_full/2)
imperm_layer_Y_position = [int(M_fi_full/2)-3, int(M_fi_full/2)-2, int(M_fi_full/2)-1, int(M_fi_full/2), int(M_fi_full/2)+1, int(M_fi_full/2)+2, int(M_fi_full/2)+3, int(M_fi_full/2)+4]

wells_coord = []
for elem in imperm_layer_Y_position:
    wells_coord.append((imperm_layer_X_position, elem))
...
ro = 1700 # 1750
g = 9.8
nu = 0.0135 # коэф-т трения
delta_w = 0.0035
P_lit_distrib = np.zeros((N_r_full, M_fi_full))
P_friction_distrib = np.zeros((N_r_full, M_fi_full))
P_obstacle = np.zeros((N_r_full, M_fi_full))
for n in range(N_r_full):
    for m in range(M_fi_full):
        P_lit_distrib[n][-1-m] = (m+1)*delta_y*g*ro + Pres
        P_friction_distrib[n][-1 - m] = ((m+1)*delta_y*g*ro + Pres)*nu*(delta_w/delta_y)
        P_obstacle[n][-1 - m] = P_lit_distrib[n][-1-m] + P_friction_distrib[n][-1 - m]
print(P_obstacle)
def PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, P_bottom_x, P_top, CP_dict, wells_frac_coords,
    ...
    for m in range(M_fi_full):
        ...
        for n in range(1, N_r_full - 1):
            ...
            if m in imperm_layer_Y_position and n == imperm_layer_X_position-1:
        ...
        for n in range(0, N_r_full):
            ...
            if m == imperm_layer_Y_position[0] - 1 and n == imperm_layer_X_position:
                A[n][n] = A[n][n] + Pres_distrib[n][m] / delta_y ** 2
                A_sym_right[n][n] = 0
            if m == imperm_layer_Y_position[-1] + 1 and n == imperm_layer_X_position:
                A[n][n] = A[n][n] + Pres_distrib[n][m] / delta_y ** 2
                A_sym_left[n][n] = 0
        ...
        for n in range(0, N_r_full):
            ...
            if m == 0 and n == 0:
                ...
                B[j][0] = -1 / 2 / alpha / delta_t * Pres_distrib[n][m] - (
                            (Pres_distrib[n][m] - Pres_distrib[n - 1][m]) / delta_x) ** 2 - (
                                      (Pres_distrib[n][m + 1] - Pres_distrib[n][m - 1]) / 2 / delta_y) ** 2
            elif m == imperm_layer_Y_position[0]-1 and n == imperm_layer_X_position:
                B[j][0] = -1 / 2 / alpha / delta_t * Pres_distrib[n][m] - (
                            (Pres_distrib[n+1][m] - Pres_distrib[n - 1][m]) /2/ delta_x) ** 2 - (
                                      (Pres_distrib[n][m] - Pres_distrib[n][m - 1]) / delta_y) ** 2
            elif m == imperm_layer_Y_position[-1]+1 and n == imperm_layer_X_position:
                B[j][0] = -1 / 2 / alpha / delta_t * Pres_distrib[n][m] - (
                            (Pres_distrib[n+1][m] - Pres_distrib[n-1][m]) /2/ delta_x) ** 2 - (
                                      (Pres_distrib[n][m + 1] - Pres_distrib[n][m]) / delta_y) ** 2
            elif m in imperm_layer_Y_position and n == imperm_layer_X_position-1:
                B[j][0] = -1 / 2 / alpha / delta_t * Pres_distrib[n][m] - (
                            (Pres_distrib[n][m] - Pres_distrib[n-1][m]) / delta_x) ** 2 - (
                                      (Pres_distrib[n][m+1] - Pres_distrib[n][m - 1]) / 2/delta_y) ** 2
            elif m in imperm_layer_Y_position and n == imperm_layer_X_position+1:
...
for t in range(T):
    ...
    for m in range(M_fi_full):
        for n in range(N_r_full):
            if Pres_distrib[n][m] >= P_obstacle[n][m]:
                print('critical P', n, m, t, Pres_distrib[n][m], P_obstacle[n][m], Pres_distrib[n][m - 1],
                      P_lit_distrib[n][m - 1])
...
for elem in wells_coord:
    ax.scatter(elem[0]*delta_x, elem[1]*delta_y, c='black', marker='|')
plt.savefig('figures/break_7_6_2.png', dpi=1000, bbox_inches='tight', pad_inches=0.1)
