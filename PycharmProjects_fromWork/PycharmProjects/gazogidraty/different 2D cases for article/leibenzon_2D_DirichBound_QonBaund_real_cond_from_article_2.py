...
delta_t = 60
Pres = 1*10**5
Pres_distrib = np.load("Pres_distrib_500000.npy")
...
for t in range(T):
    ...
    if t == 100000 or t == 150000 or t == 200000 or t == 250000 or t == 300000 or t == 350000 or t == 400000 or t == 450000 or t == 500000:
        np.save('Pres_distrib_500000_plus_'+str(t), Pres_distrib)
...
surf = plt.contourf(X_list, Y_list, np.rot90(Pres_distrib, 3), cmap=cm.jet, antialiased=True, vmin=np.nanmin(P_list), vmax=np.nanmax(P_list),linewidth=0.2)
ax.set_aspect(0.5)
fig.colorbar(surf, shrink=0.5, aspect=5)
plt.show()