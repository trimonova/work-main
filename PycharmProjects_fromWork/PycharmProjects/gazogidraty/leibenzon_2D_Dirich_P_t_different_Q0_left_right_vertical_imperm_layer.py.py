...
for t in range(T):
    for n in range(N_r_full):
        if n == round(N_r_full/2) or n == round(N_r_full/2)+1 or n == round(N_r_full/2)+2:
            P_bottom_x.append(1.115*10**5) # если поставить 1.11 то будет разрыв в одной точке соседней с полоской
        else:
            P_bottom_x.append(1.06*10**5) # если везде поставить 1.068 или 1.0685, то разрыв будет только у полоски
    P_bottom_x_t.append(P_bottom_x)

#P_bottom_t = [1.066*10**5 for i in range(T)]
viscosity_matrix = np.ones((N_r_full, M_fi_full))*mu_water

imperm_layer_X_position = int(N_r_full/2)-3
