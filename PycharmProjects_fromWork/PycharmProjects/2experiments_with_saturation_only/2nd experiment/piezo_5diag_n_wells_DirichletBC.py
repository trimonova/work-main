...
def PorePressure_in_Time(N_r, M_fi, Pres_distrib, c1, c2, c3_water, c4, wells_coord, CP_dict, delta_r, delta_fi, r_well):
    ...
    for n in range(1, N_r - 1):
        A[n][n - 1] = c1 - c2 / ((n + 1) * delta_r + r_well)
        A[n][n] = -2 * c1 - c3_water - 2/((n+1)*delta_r + r_well)**2/delta_fi**2
        A[n][n + 1] = c1 + c2 / ((n + 1) * delta_r + r_well)
    ...
    A[0][0] = -2 * c1 - c3_water - 2/(delta_r + r_well)**2/delta_fi**2 + (c1 - c2/(delta_r + r_well))
    A[0][1] = c1 + c2 / (1 * delta_r + r_well)
    A[N_r - 1][N_r - 1] = -2 * c1 - c3_water + c1 + c2 / ((N_r) * delta_r + r_well) - 2/((N_r)*delta_r + r_well)**2/delta_fi**2
    A[N_r - 1][N_r - 2] = c1 - c2 / ((N_r) * delta_r + r_well)

    A_sym = np.zeros((N_r, N_r))
    for n in range(0,N_r):
        A_sym[n][n] = c4/((n+1)*delta_r + r_well)**2
...
if __name__ == '__main__':
    ...
    for t in range(T_exp):
        print(t)
        Pres_distrib, A_full, B = PorePressure_in_Time(N_r, M_fi, Pres_distrib, c1, c2, c3_water, c4, wells_coord, CP_dict, delta_r, delta_fi, r_well)
