...
def PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, c3_oil, c3_water, CP_dict, P_center, wells_frac_coords,
    ...
    for m in range(M_fi_full):
        ...
        A[0][0] = -2*c1 - c3_oil - 2/(delta_r_list[0]+r_well)**2/delta_fi_list[m]**2
        ...
        for n in range(0, N_r_full):
            A_sym[n][n] = c4 / (sum(delta_r_list[0:n])+r_well) ** 2
    ...
    for m in range(M_fi_full):
        for n in range(N_r_full):
            if n == 0:
                c1 = 1 / delta_r_list[n]**2
                c2 = 1 / 2 / delta_r_list[n]
                B[j][0] = -c3_oil * Pres_distrib[n][m] - (c1 - c2 / (delta_r_list[n] + r_well))*P_center
            else:
                k_fluid = viscosity_matrix[n][m] * fi * C_total / perm
                c3_fluid = k_fluid / delta_t
