...
def define_func_matrix(pressure_field, func_matrix, perm, delta_r_list, delta_fi_list, t_step, r_well, q, viscosity_matrix):
    ...
    for m in range(0, M_fi_full):
        for n in range(0, N_r_full):
            if n == 0 and m != 0 and m != M_fi_full-1:
                ...
                #V_r = -perm / viscosity_matrix[n][m] * (pressure_field[n][m] - Pinj) / delta_r_list[n]
                V_r = q
                V_fi = -perm / viscosity_matrix[n][m] * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[m] / (sum(
                ...
                df_dfi = (func_matrix[n][m+1] - func_matrix[n][m - 1]) / (sum(delta_r_list[0:n])+r_well) / delta_fi_list[m]/2
                V_r = -perm / viscosity_matrix[n][m] * (pressure_field[n][m] - pressure_field[n-1][m]) / delta_r_list[
                        n]
                V_fi = -perm / viscosity_matrix[n][m] * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[
                ...
                df_dfi = (func_matrix[n][m + 1] - func_matrix[n][m]) / (sum(delta_r_list[0:n])+r_well) / \
                               delta_fi_list[m]
                V_r = -perm / viscosity_matrix[n][m] * (pressure_field[n + 1][m] - pressure_field[n][m]) / delta_r_list[
                        n]
                V_fi = -perm / viscosity_matrix[n][m] * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[
                        m] / (sum(delta_r_list[0:n])+r_well)
                ...
                V_r = -perm / viscosity_matrix[n][m] * (pressure_field[n + 1][m] - pressure_field[n][m]) / delta_r_list[
                        n]
                V_fi = -perm / viscosity_matrix[n][m] * (pressure_field[n][m] - pressure_field[n][m-1]) / delta_fi_list[
                        m] / (sum(delta_r_list[0:n])+r_well)
                ...
                #V_r = -perm / viscosity_matrix[n][m] * (pressure_field[n + 1][m] - Pinj) / delta_r_list[n]
                V_r = q
                V_fi = -perm / viscosity_matrix[n][m] * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[
                        m] / (sum(delta_r_list[0:n])+r_well)
                ...
                V_r = -perm / viscosity_matrix[n][m] * (pressure_field[n][m] - pressure_field[n-1][m]) / delta_r_list[n]
                V_fi = -perm / viscosity_matrix[n][m] * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[
                        m] / (sum(delta_r_list[0:n])+r_well)
            ...
            if np.isnan(func_matrix_new[n][m]):
                ...
                print(velocity[n][m], V_r, df_dr_left, df_dr_right, viscosity_matrix[n][m], pressure_field[n - 1][m], pressure_field[n][m], delta_r_list[n])
                print(df_dr, df_dfi)
                print(V_r, V_fi)
                raise ValueError
    # new_bound = []
    ...
    return func_matrix_new, velocity
