...
from newFuncMatrix_fix_5 import define_func_matrix
import copy
...
from find_pore_pressure_PinCenter import PorePressure_in_Time
...
coord_matrix = np.array(coord_matrix_cart)


delta_t = 0.5
Pres = 1*10**5
P_center = 60*10**5
P_frac_right = [P_center for i in range(N_1)]
P_frac_left = [P_center for i in range(N_2)]
...
P_well = [1 for i in range(N_1+N_2)]
...
for i in range(len(frac_coords)):
    CP_dict[frac_coords[i]] = P_well[i]
...
surf = plt.contourf(X, Y, viscosity_matrix, linewidth=0.2, cmap=plt.get_cmap('jet'))
plt.title('viscosity')
plt.colorbar(surf)
plt.show()
#plt.colorbar(surf, ax=axs[0, 0])

surf_1 = plt.contourf(X, Y, Func_matrix_remake, linewidth=0.2, cmap=plt.get_cmap('jet'))
...
if __name__ == '__main__':
    ...
    for t in range(T_exp):
        print(t)
        plt.plot(viscosity_matrix[:, M_1])
        plt.plot(viscosity_matrix[:, M_2])
        plt.show()
        plt.plot(viscosity_matrix[:, M_1 - 1])
        plt.plot(viscosity_matrix[:, M_1 + 1])
        plt.show()
        plt.plot(viscosity_matrix[:, M_1 + 2])
        plt.plot(viscosity_matrix[:, M_1 - 2])
        plt.show()
        plt.plot(viscosity_matrix[:, M_2 + 2])
        plt.plot(viscosity_matrix[:, M_2 - 2])
        plt.show()
        Pres_distrib, A, B = PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, c3_oil, c3_water, CP_dict, P_frac_right, P_frac_left, frac_coords, delta_r_list, delta_fi_list, viscosity_matrix, porosity, C_total, perm, delta_t,
                         r_well, M_1, M_2, N_1, N_2)

        plt.plot(Pres_distrib[:, M_1])
        plt.plot(Pres_distrib[:, M_2])
        plt.show()
        plt.plot(Pres_distrib[:, M_1-1])
        plt.plot(Pres_distrib[:, M_1+1])
        plt.show()
        plt.plot(Pres_distrib[:, M_1+2])
        plt.plot(Pres_distrib[:, M_1-2])
        plt.show()
        plt.plot(Pres_distrib[:, M_2 + 2])
        plt.plot(Pres_distrib[:, M_2 - 2])
        plt.show()
        #
        ...
        Func_matrix, velocity = define_func_matrix(Pres_distrib, Func_matrix_remake, perm, delta_r_list, delta_fi_list, delta_t, r_well, viscosity_matrix,  N_r_full, M_fi_full)
        ...
        plt.plot(Func_matrix_remake[int(N_1/2), :])
        #plt.show()
    ...
    dataFrame_to_study.to_csv('C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\QinPackers\\dataframes\\dataframe.csv')
    plt.figure()
    plt.plot(velocity[:, int(M_fi_full / 8 * 3)])
    plt.show()
