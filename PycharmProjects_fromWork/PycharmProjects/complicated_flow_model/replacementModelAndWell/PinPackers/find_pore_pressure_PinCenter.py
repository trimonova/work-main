...
def PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, c3_oil, c3_water, CP_dict, P_frac_right, P_frac_left, wells_frac_coords,
    ...
    for m in range(M_fi_full):
        ...
        for n in range(1, N_r_full - 1):
            ...
            A[n][n - 1] = c1 - c2 / (sum(delta_r_list[0:n])+r_well)

            A[n][n] = -2 * c1 - c3_fluid - 2 / (sum(delta_r_list[0:n])+r_well) ** 2 / delta_fi_list[m] ** 2
        ...
        A[N_r_full - 1][N_r_full - 1] = -2 * c1 - c3_water + c1 + c2 / (sum(delta_r_list[0:N_r_full-1])+r_well) - 2/(sum(delta_r_list[0:N_r_full-1])+r_well)**2/delta_fi_list[m]**2
        A[N_r_full - 1][N_r_full - 2] = c1 - c2 / (sum(delta_r_list[0:N_r_full-1])+r_well)
        ...
        for n in range(0, N_r_full):
            A_sym_right[n][n] = c4 / (sum(delta_r_list[0:n])+r_well) ** 2
            A_sym_left[n][n] = c4 / (sum(delta_r_list[0:n]) + r_well) ** 2

            if (m == M_1 + 1 and n < N_1):
                #A[n][n] = A[n][n] - 1 / (sum(delta_r_list[0:n]) + r_well) ** 2 / delta_fi_list[m] ** 2
                A_sym_left[n][n] = 0
                ...
                #c2 = 1 / 2 / delta_r_list[n]
                A[n-1][n] = 0
                #A[n][n] = A[n][n] + (c1 - c2 / (sum(delta_r_list[0:n]) + r_well))
    ...
    for m in range(M_fi_full):
        for n in range(N_r_full):
            ...
            c4 = 1 / delta_fi_list[m] ** 2
            A_sym = c4 / (sum(delta_r_list[0:n]) + r_well) ** 2
            if (m == M_1 + 1 and n < N_1):
                #B[j][0] = B[j][0] - 1 / (sum(delta_r_list[0:n]) + r_well) / delta_fi_list[m] * (viscosity_matrix[n][m]*q/perm)
                B[j][0] = B[j][0] - A_sym*P_frac_right[n]
            elif m == M_2 + 1 and n < N_2:
                #B[j][0] = B[j][0] - 1 / (sum(delta_r_list[0:n]) + r_well) / delta_fi_list[m] * (viscosity_matrix[n][m]*q/perm)
                B[j][0] = B[j][0] - A_sym * P_frac_left[n]
            elif m == M_1 - 1 and n < N_1:
                #B[j][0] = B[j][0] - 1 / (sum(delta_r_list[0:n]) + r_well) / delta_fi_list[m] * (viscosity_matrix[n][m]*q/perm)
                B[j][0] = B[j][0] - A_sym * P_frac_right[n]
            elif m == M_2 - 1 and n < N_2:
            ...
            if m == M_1 and n == N_1:
                c1 = 1 / delta_r_list[n] ** 2
                c2 = 1 / 2 / delta_r_list[n]
                B[j][0] = B[j][0] - (c1 - c2 / (sum(delta_r_list[0:n]) + r_well))*P_frac_right[n-1]
            if m == M_2 and n == N_2:
                c1 = 1 / delta_r_list[n] ** 2
                c2 = 1 / 2 / delta_r_list[n]
                B[j][0] = B[j][0] - (c1 - c2 / (sum(delta_r_list[0:n]) + r_well))*P_frac_left[n-1]
    ...
    for coord_couple in wells_frac_coords:
        ...
        #P_new_along_the_fracture_5.append(P_new[(M+2) * N_r_full + N])
        #P_new_along_the_fracture_6.append(P_new[(M - 2) * N_r_full + N])

    #plt.plot(P_new_along_the_fracture_6)
    # plt.plot(P_new_along_the_fracture_2)
    # plt.plot(P_new_along_the_fracture_1)
    # plt.plot(P_new_along_the_fracture_4)
    # plt.show()
    #plt.plot(P_new_along_the_fracture_5)
