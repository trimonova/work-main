...
wells_coord = [(round(0.1392/delta_r), round(np.pi*0.366/delta_fi)), (round(0.1392/delta_r), round((np.pi+np.pi*0.366)/delta_fi))]
P_well = [1500000, 0]
print(wells_coord)
...
for t in range(T_exp_dir):
    print(t)
    Pres_distrib, A, B = PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, c3_oil, c3_water, CP_dict, q_coef,
...
plt.plot(t_list, np.array(PinPoint5)/10**6)
plt.show()

plt.plot(t_list_exp, P_filter_6)
plt.plot(t_list, np.array(PinPoint6)/10**6)
plt.show()

plt.plot(t_list_exp, P_filter_7)
plt.plot(t_list, np.array(PinPoint7)/10**6)
plt.show()

plt.plot(t_list_exp, P_filter_8)
plt.plot(t_list, np.array(PinPoint8)/10**6)
plt.show()
...
plt.plot(t_list_exp, P_filter_11)
plt.plot(t_list, np.array(PinPoint11)/10**6)
plt.show()

plt.plot(t_list_exp, P_filter_12)
plt.plot(t_list, np.array(PinPoint12)/10**6)
plt.show()

plt.plot(t_list_exp, P_filter_13)
...
from QinCenter.newFuncMatrix_fix_4 import define_func_matrix
import copy
from QinCenter.find_viscosity import find_viscosity

from QinCenter.find_bound_coords import find_bound_coords
from QinCenter.find_func_matrix_remake import find_func_matrix_remake
from QinCenter.find_pore_pressure_QinCenter import PorePressure_in_Time
import pandas as pd
...
# P_center = 60*10**5
Q_center = 1.0 * 10 ** (-9)  # из лаб. данных - 0.2*10**(-6) m3/s
s = 4 * 10 ** (-5)  # ширина прорези 2 мм, высота - 1 см
q = Q_center / s  # m/s
q_coef = q * delta_r_fine * mu_oil / perm
...
for t in range(T_exp):
    ...
    Func_matrix, velocity = define_func_matrix(Pres_distrib, Func_matrix_remake, perm, delta_r_list, delta_fi_list,
                                               delta_t, r_well, q, viscosity_matrix)
    ...
    if t == 9 or t == 10 or t == 19 or t == 20 or t == 49 or t == 50 or t == 89 or t == 90 or t == 88:
        ...
        dataFrame_vel.to_csv(
            'C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframes_QinCenter\\dataframe_vel_t_' + str(
                t) + '.csv')
        dataFrame_visc.to_csv(
            'C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframes_QinCenter\\dataframe_visc_t_' + str(
                t) + '.csv')
        dataFrame_func_matrix.to_csv(
            'C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframes_QinCenter\\dataframe_func_matr_t_' + str(
                t) + '.csv')
        dataFrame_func_matrix_remake.to_csv(
            'C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframes_QinCenter\\dataframe_func_matr_rem_t_' + str(
                t) + '.csv')
        dataFrame_Pres_distrib.to_csv(
            'C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframes_QinCenter\\dataframe_P_res_t_' + str(
                t) + '.csv')
...
#     'C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframes_QinCenter\\dataframe.csv')
plt.figure()
