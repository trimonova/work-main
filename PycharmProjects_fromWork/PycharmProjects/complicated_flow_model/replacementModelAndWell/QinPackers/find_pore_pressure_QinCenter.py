...
def PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, c3_oil, c3_water, CP_dict, q, wells_frac_coords,
    ...
    for m in range(M_fi_full):
        for n in range(N_r_full):
            ...
            if m == M_1 and n == N_1:
                c1 = 1 / delta_r_list[n] ** 2
                c2 = 1 / 2 / delta_r_list[n]
                B[j][0] = B[j][0] - (c1 - c2 / (sum(delta_r_list[0:n]) + r_well))*delta_r_list[n]*viscosity_matrix[n][m]*q/perm
            if m == M_2 and n == N_2:
                c1 = 1 / delta_r_list[n] ** 2
                c2 = 1 / 2 / delta_r_list[n]
                B[j][0] = B[j][0] - (c1 - c2 / (sum(delta_r_list[0:n]) + r_well))*delta_r_list[n]*viscosity_matrix[n][m]*q/perm
