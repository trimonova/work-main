...
from find_pore_pressure_QinCenter_test import PorePressure_in_Time
...
if __name__ == '__main__':
    ...
    for t in range(T_exp):
        ...
        plt.plot(viscosity_matrix[:, M_2 - 1])
        plt.show()
        plt.plot(viscosity_matrix[:, M_1 + 1])
        plt.plot(viscosity_matrix[:, M_2 + 1])
        plt.show()
        ...
        plt.plot(viscosity_matrix[:, M_1 - 2])
        plt.show()
        plt.plot(viscosity_matrix[:, M_1 + 3])
        plt.plot(viscosity_matrix[:, M_1 - 3])
        plt.show()

        Pres_distrib, A, B = PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, c3_oil, c3_water, CP_dict, q, frac_coords, delta_r_list, delta_fi_list, viscosity_matrix, porosity, C_total, perm, delta_t,
        ...
        Func_matrix, velocity = define_func_matrix(Pres_distrib, Func_matrix_remake, perm, delta_r_list, delta_fi_list, delta_t, r_well, q, viscosity_matrix,  N_r_full, M_fi_full)
