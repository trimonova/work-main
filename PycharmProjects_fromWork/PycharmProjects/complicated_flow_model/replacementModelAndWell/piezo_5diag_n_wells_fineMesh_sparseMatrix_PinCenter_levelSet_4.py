...
from find_pore_pressure import PorePressure_in_Time
...
if __name__ == '__main__':
    ...
    for t in range(T_exp):
        print(t)
        Pres_distrib, A, B = PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, c3_oil, c3_water, CP_dict, P_center, wells_coord, delta_r_list, delta_fi_list, viscosity_matrix, porosity, C_total, perm, delta_t,
                         r_well)
        print(min(Pres_distrib.flat), max(Pres_distrib.flat))
        Func_matrix, velocity = define_func_matrix(Pres_distrib, Func_matrix_remake, perm, delta_r_list, delta_fi_list, delta_t, r_well, P_center, viscosity_matrix)
        ...
        if t == 58 or t == 59 or t == 60 or t == 69 or t == 70 or t == 79 or t == 80 or t == 68 or t == 78:
            ...
            dataFrame_vel.to_csv(
                'C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframe_vel_t_'+str(t)+'.csv')
            dataFrame_visc.to_csv(
                'C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframe_visc_t_'+str(t)+'.csv')
            dataFrame_func_matrix.to_csv(
                'C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframe_func_matr_t_'+str(t)+'.csv')
            dataFrame_func_matrix_remake.to_csv(
                'C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframe_func_matr_rem_t_'+str(t)+'.csv')
    ...
    dataFrame_to_study.to_csv('C:\disk_D\Repos\PycharmProjects_fromWork\PycharmProjects\complicated_flow_model\\replacementModelAndWell\\dataframe.csv')
    plt.figure()
    plt.plot(velocity[:, int(M_fi_full / 8 * 3)])
    plt.show()
