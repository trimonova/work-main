...
def define_func_matrix(pressure_field, func_matrix, perm, mu_water, mu_oil, delta_r_list, delta_fi_list, t_step, r_well, Pinj):
    ...
    df_dr_left = np.zeros((N_r_full, M_fi_full))
    df_dr_right = np.zeros((N_r_full, M_fi_full))
    df_dfi_left = np.zeros((N_r_full, M_fi_full))
    df_dfi_right = np.zeros((N_r_full, M_fi_full))
    V_r = np.zeros((N_r_full, M_fi_full))
    V_fi = np.zeros((N_r_full, M_fi_full))
    for m in range(1, M_fi_full-1):
        df_dr_right[0][m] = (func_matrix[n + 1][m] - func_matrix[n][m]) / delta_r_list[n]
        df_dr_left[0][m] = df_dr_right
        df_dfi_left[0][m] = (func_matrix[n][m + 1] - func_matrix[n][m - 1]) / (sum(delta_r_list[0:n]) + r_well) / \
                            delta_fi_list[
                                m] / 2
        df_dfi_right[0][m] = (func_matrix[n][m + 1] - func_matrix[n][m]) / sum(delta_r_list[0:n]) / \
                             delta_fi_list[m]
        df_dr_right[N_r_full - 1][m] = (func_matrix[n + 1][m] - func_matrix[n][m]) / delta_r_list[n]
        df_dr_left[N_r_full - 1][m] = df_dr_right
        df_dfi_left[N_r_full - 1][m] = (func_matrix[n][m + 1] - func_matrix[n][m - 1]) / (
                    sum(delta_r_list[0:n]) + r_well) / \
                                       delta_fi_list[
                                           m] / 2
        df_dfi_right[N_r_full - 1][m] = (func_matrix[n][m + 1] - func_matrix[n][m]) / sum(delta_r_list[0:n]) / \
                                        delta_fi_list[m]
        if func_matrix[0][m] < 0:
            V_r[0][m] = -perm / mu_water * (pressure_field[n][m] - Pinj) / delta_r_list[n]
            V_fi[0][m] = -perm / mu_water * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[m] / (sum(
                delta_r_list[0:n]) + r_well)
        else:
            V_r[0][m] = -perm / mu_oil * (pressure_field[n][m] - Pinj) / delta_r_list[
                n]
            V_fi[0][m] = -perm / mu_oil * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[
                m] / (sum(delta_r_list[0:n]) + r_well)
        if func_matrix[N_r_full-1][m] < 0:
            V_r[N_r_full-1][m] = -perm / mu_water * (pressure_field[n][m] - Pinj) / delta_r_list[n]
            V_fi[N_r_full-1][m] = -perm / mu_water * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[m] / (
                        sum(
                            delta_r_list[0:n]) + r_well)
        else:
            V_r[N_r_full-1][m] = -perm / mu_oil * (pressure_field[n][m] - Pinj) / delta_r_list[
                n]
            V_fi[N_r_full-1][m] = -perm / mu_oil * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[
                m] / (sum(delta_r_list[0:n]) + r_well)

        for n in range(1, N_r_full-1):
            df_dr_left[n][m] = (func_matrix[n][m] - func_matrix[n - 1][m]) / delta_r_list[n]
            df_dr_right[n][m] = (func_matrix[n + 1][m] - func_matrix[n][m]) / delta_r_list[n]
            df_dfi_left[n][m] = (func_matrix[n][m] - func_matrix[n][m - 1]) / sum(delta_r_list[0:n]) / delta_fi_list[m]
            df_dfi_right[n][m] = (func_matrix[n][m + 1] - func_matrix[n][m]) / sum(delta_r_list[0:n]) / \
                           delta_fi_list[m]
            df_dr = (func_matrix[n + 1][m] - func_matrix[n - 1][m]) / delta_r_list[n] / 2
            df_dfi = (func_matrix[n][m + 1] - func_matrix[n][m - 1]) / (sum(delta_r_list[0:n]) + r_well) / \
                     delta_fi_list[m] / 2


            if func_matrix[n][m] < 0:
                V_r[n][m] = -perm / mu_water * (pressure_field[n + 1][m] - pressure_field[n][m]) / delta_r_list[n]
                V_fi[n][m] = -perm / mu_water * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[m] / (
                            sum(delta_r_list[0:n]) + r_well)
            else:
                V_r[n][m] = -perm / mu_oil * (pressure_field[n + 1][m] - pressure_field[n][m]) / delta_r_list[
                    n]
                V_fi[n][m] = -perm / mu_oil * (pressure_field[n][m + 1] - pressure_field[n][m]) / delta_fi_list[
                    m] / (sum(delta_r_list[0:n]) + r_well)
