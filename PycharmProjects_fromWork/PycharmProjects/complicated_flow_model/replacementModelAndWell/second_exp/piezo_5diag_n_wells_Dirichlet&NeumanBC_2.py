...
plt.plot(t_list_exp, P_filter_9)
plt.plot(t_list, np.array(PinPoint13)/10**6)
plt.show()
...
# P_center = 60*10**5
Q_center = 0.2 * 10 ** (-9)  # из лаб. данных - 0.2*10**(-6) m3/s
s = 4 * 10 ** (-5)  # ширина прорези 2 мм, высота - 1 см
q = Q_center / s  # m/s
q_coef = q * delta_r_fine * mu_oil / perm
...
axs[2, 1].plot(t_list_exp, P_filter_10*10**6)
axs[2, 1].plot(t_list, PinPoint10)
