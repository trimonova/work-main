...
from func_to_define_dist_findBoundAutomat import replace_boundary
from newFuncMatrix_fix_findBoundAutomat import define_func_matrix
...
Func_matrix_remake = Func_matrix.copy()
...
def PorePressure_in_Time(N_r_full, M_fi_full, Pres_distrib, c3_oil, c3_water, CP_dict, P_center, wells_frac_coords, delta_r, delta_fi, Func_matrix):
    ...
    for m in range(M_fi_full):
        ...
        if m == 0:
            try:
                A_zero_coo = coo_matrix((N_r_full, N_r_full * M_fi_full - 3 * N_r_full))
                A_line_1 = hstack([A_coo, A_sym_coo, A_zero_coo, A_sym_coo])
            except MemoryError:
                print(memory_usage())
                raise ValueError
            ...
            A_zero_coo = coo_matrix((N_r_full, N_r_full * M_fi_full - 3 * N_r_full))
            A_line_end = hstack([A_sym_coo, A_zero_coo, A_sym_coo, A])
            A_full = vstack([A_full, A_line_end])
        else:
            A_zero_coo_1 = coo_matrix((N_r_full, N_r_full * (m - 1)))
            A_zero_coo_2 = coo_matrix((N_r_full, N_r_full * M_fi_full - (3 + (m - 1)) * N_r_full))
            A_line = hstack([A_zero_coo_1, A_sym_coo, A_coo, A_sym_coo, A_zero_coo_2])
...
if __name__ == '__main__':
    ...
    for t in range(T_exp):
        ...
        Func_matrix, velocity, new_bound_G, Func_matrix_remake, line, area = define_func_matrix(Pres_distrib, Func_matrix_remake, perm, mu_water, mu_oil, delta_r_list, delta_fi_list, delta_t, r_well, P_center, bound_coord, line, area, Func_coord_dict)
        bound_coord = new_bound_G.copy()
        #print(min(Func_matrix.flat), max(Func_matrix.flat))
        #print(min(Func_matrix_remake.flat), max(Func_matrix_remake.flat))
        print(min(velocity.flat), max(velocity.flat))
        print('velocity on the bound - ', velocity[int(0.005 / delta_r_fine) + 2, int(M_fi_full / 8 * 4)])
        print('pressure on the bound 1 - ', Pres_distrib[int(0.005 / delta_r_fine) + 2, int(M_fi_full / 8 * 4)])
        print('pressure on the bound 2 - ', Pres_distrib[int(0.005 / delta_r_fine) + 1, int(M_fi_full / 8 * 4)])
        #plt.plot(Pres_distrib[:, int(M_fi_full / 2)])
        #plt.plot(velocity[:, int(M_fi_full / 2)])
    ...
    plt.plot(Pres_distrib[:, int(M_fi_full/8*4)])
    ...
    plt.show()
    fig2 = plt.figure()
    print(velocity[int(0.05/delta_r_fine)-2, int(M_fi_full/8*3)])
    plt.plot(velocity[:, int(M_fi_full/8*4)])
    plt.xlabel("Номер ячейки")
    ...
    Func_matrix_old_list = [l for l in Func_matrix.flat]
    ...
    bound_old_i = interpolate.griddata((X_list, Y_list), Func_matrix_old_list, (xig, yig), method='cubic')
    velocity_i = interpolate.griddata((X_list, Y_list), velocity_list, (xig, yig), method='cubic')
    bound_old_i_notNan = []
    bound_i_notNan = []
    velocity_i_notNan = []
    for elem in bound_old_i.flat:
        if not np.isnan(elem):
            bound_old_i_notNan.append(elem)

    for elem in bound_i.flat:
        if not np.isnan(elem):
            bound_i_notNan.append(elem)

    for elem in velocity_i.flat:
        if not np.isnan(elem):
            velocity_i_notNan.append(elem)

    print(max(bound_old_i_notNan), min(bound_old_i_notNan))
    print(max(bound_i_notNan), min(bound_i_notNan))
    print(max(velocity_i_notNan), min(velocity_i_notNan))
    ...
    fig = plt.figure()
    surf = plt.contourf(xig, yig, bound_old_i, linewidth=0.2, cmap=plt.get_cmap('jet'))
    fig.colorbar(surf)
    # plt.scatter(new_bound_G[0], new_bound_G[1], marker='.')
    plt.show()
    ...
    fig = plt.figure()
    surf = plt.contourf(xig, yig, velocity_i, linewidth=0.2, cmap=plt.get_cmap('jet'))
    fig.colorbar(surf)
    plt.show()
